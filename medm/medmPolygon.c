...
static void polygonUpdateValueCb(Channel *pCh);
static void polygonDestroyCb(Channel *pCh);


static void drawPolygon(Channel *pCh) {
  DisplayInfo *displayInfo = pCh->displayInfo;
  Display *display = XtDisplay(pCh->displayInfo->drawingArea);
  DlPolygon *dlPolygon = (DlPolygon *) pCh->specifics;
                                ...
                                Boolean forcedDisplayToWindow)
{
  if ((displayInfo->traversalMode == DL_EXECUTE) 
    ...
    *pCh->dlAttr =displayInfo->attribute;
    ...
    pCh->monitorType = DL_Polygon;
    pCh->specifics = (XtPointer) dlPolygon;
    pCh->updateChannelCb = polygonUpdateValueCb;
    pCh->updateGraphicalInfoCb = NULL;
    pCh->destroyChannel = polygonDestroyCb;
  ...
  if (ca_state(pCh->chid) == cs_conn) {
    ...
    switch (pCh->vismod) {
      ...
      case IF_ZERO:
        ...
        break;
      default :
        medmPrintf("internal error : polygonUpdateValueCb : unknown visibility attribute\n");
        break;
    }
