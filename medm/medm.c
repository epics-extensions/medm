  ...
  if (request->medmMode != LOCAL) {
    ...
    if (request->fontStyle == FIXED) {
      if (request->opMode == EXECUTE) {
        MEDM_EXEC_FIXED = XInternAtom(display,"MEDM020202_EXEC_FIXED",False);
        ...
        MEDM_EDIT_FIXED = XInternAtom(display,"MEDM020202_EDIT_FIXED",False);
    ...
    if (request->fontStyle == SCALABLE) {
      if (request->opMode == EXECUTE) {
        MEDM_EXEC_SCALABLE = XInternAtom(display,"MEDM020202_EXEC_SCALABLE",False);
        status = XGetWindowProperty(display,rootWindow,MEDM_EXEC_SCALABLE,
		0,FULLPATHNAME_SIZE,(Bool)False,AnyPropertyType,&type,
		&format,&nitems,&left,&propertyData);
      } else {
        MEDM_EDIT_SCALABLE = XInternAtom(display,"MEDM020202_EDIT_SCALABLE",False);
