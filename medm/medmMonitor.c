  ...
  do
    {
      switch ((tokenType = getToken(displayInfo, token)))
        ...
        case T_WORD:
          ...
          else if (!strcmp(token, "y2label"))
            {
              getToken(displayInfo, token);
              getToken(displayInfo, token);
              strcpy(plotcom->y2label, token);
            }
          else if (!strcmp(token, "y3label"))
            {
              getToken(displayInfo, token);
              getToken(displayInfo, token);
              strcpy(plotcom->y3label, token);
            }
          else if (!strcmp(token, "y4label"))
            {
              getToken(displayInfo, token);
              getToken(displayInfo, token);
              strcpy(plotcom->y4label, token);
            }
  ...
  do
    {
      switch ((tokenType = getToken(displayInfo, token)))
        ...
        case T_WORD:
          ...
          else if (!strcmp(token, "yside"))
            {
              getToken(displayInfo, token);
              getToken(displayInfo, token);
              trace->yside = atoi(token);
#endif
            }
          break;
        case T_LEFT_BRACE:
  ...
  if (MedmUseNewFileFormat)
    {
      ...
      if (dlPlotcom->y2label[0] != '\0')
        fprintf(stream, "\n%s\ty2label=\"%s\"", indent, dlPlotcom->y2label);
      if (dlPlotcom->y3label[0] != '\0')
        fprintf(stream, "\n%s\ty3label=\"%s\"", indent, dlPlotcom->y3label);
      if (dlPlotcom->y4label[0] != '\0')
        fprintf(stream, "\n%s\ty4label=\"%s\"", indent, dlPlotcom->y4label);
...
#if defined(SCIPLOT)
  fprintf(stream, "\n%s\tyside=%d", indent, dlTrace->yside);
#endif
  fprintf(stream, "\n%s}", indent);
}
  ...
  if (MedmUseNewFileFormat)
    {
      ...
      if ((dlPlotAxisDefinition->axisStyle == LINEAR_AXIS) &&
          ...
          (dlPlotAxisDefinition->maxRange == 1.0))
        return;
#ifdef SUPPORT_0201XX_FILE_FORMAT
    }
#endif
