  ...
  for (i = 0; i < sccd.numYAxisLabel; i++)
    {
      ...
      if (width > maxWidth)
        maxWidth = width;
      if ((int)range[i].numDot > maxDot)
        maxDot = range[i].numDot;
    }
...
{
  sccd.yAxisLabelHeight = sccd.axisLabelFontHeight * sccd.numYAxisLabel + psc->dataHeight;
}
...
static void stripChartConfig(MedmStripChart *psc)
{
  ...
  if ((widthExt > (sccd.margin + sccd.markerHeight)) && (height > (sccd.margin + sccd.markerHeight)))
    {
      /* shrink the graph */
  ...
  if (psc->dataHeight < psc->h / 2U)
    {
      squeezeSpace = True;
      if (strlen(dlStripChart->plotcom.title) == 0)
        {
          //dropTitleLabel = True;
      ...
      if (strlen(dlStripChart->plotcom.xlabel) == 0)
        {
      ...
      if (strlen(dlStripChart->plotcom.ylabel) == 0)
        {
  ...
  XSetFont(display, gc, fontTable[sccd.axisLabelFont]->fid);
  {
    ...
    labelHeight = (int)(((double)sccd.numYAxisLabel + verticalSpacing) * (double)sccd.axisLabelFontHeight);
                    ...
                    if (range[j].mask & (0x1 << k))
                      {
                        ...
                        x = psc->dataX0 - sccd.yAxisLabelTextWidth -
                          sccd.markerHeight - sccd.margin - sccd.yAxisLabelOffset;
    ...
    if (dropXAxisUnitLabel == False)
      {
        ...
        if (strlen(dlStripChart->plotcom.xlabel) > (size_t)0)
          {
  ...
  if (!dlElement->widget)
    {
      ...
      n = 0;
      XtSetArg(args[n], XmNx, (Position)dlStripChart->object.x);
      n++;
      XtSetArg(args[n], XmNy, (Position)dlStripChart->object.y);
      n++;
      XtSetArg(args[n], XmNwidth, (Dimension)dlStripChart->object.width);
      n++;
      XtSetArg(args[n], XmNheight, (Dimension)dlStripChart->object.height);
      n++;
      ...
      n++;
      XtSetArg(args[n], XmNforeground, (Pixel)displayInfo->colormap[dlStripChart->plotcom.clr]);
      n++;
      XtSetArg(args[n], XmNbackground, (Pixel)displayInfo->colormap[dlStripChart->plotcom.bclr]);
      n++;
...
static void stripChartDialogCreateDialog(void)
{
  ...
  if (stripChartS != NULL || mainShell == NULL)
    return;
