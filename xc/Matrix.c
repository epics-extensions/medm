 ...
 * Max and Min are also defined in Xm/XmP.h.
 */
#ifndef Max
...
ColumnLabelLines lines;
{
    ...
    /*
     * Handle the case of one line (no index() needed)
     */
    if (lines->lines == 1)
...
XbaeMatrixWidget mw;
{
	...
	((Widget)mw, mw->matrix.current_row,
		mw->matrix.current_column, cell);
 ...
 */
static Boolean CommitEdit(mw, unmap)
  XbaeMatrixWidget mw;
  Boolean unmap;
{
    Boolean commit;
...
extern Boolean XbaeMatrixCommitEdit(Widget w, Boolean unmap)
#else
extern Boolean XbaeMatrixCommitEdit(w, unmap)
  Widget w;
  Boolean unmap;
#endif
{
 ...
 */
static void CancelEdit(mw, unmap)
  XbaeMatrixWidget mw;
  Boolean unmap;
{
    if (!XtIsManaged(TextChild(mw)))
...
#else
void XbaeMatrixCancelEdit(w, unmap)
  Widget w;
  Boolean unmap;
#endif
{
...
int num_rows;
{
	...
	((Widget)w, position, rows, labels, colors, num_rows);
}
...
int num_columns;
{
	...
	((Widget)w, position, columns, labels, widths,
...
int num_columns;
{
	...
	XClearArea(XtDisplay(mw), XtWindow(mw),
		   ...
		   0 /*Full Width*/, 0 /*Full Height*/,
		   True);
	XbaeClipRedraw(ClipChild((Widget)mw));
    }
}
...
int num_colors;
{
	...
	((Widget)w, position, colors, num_colors);
}
